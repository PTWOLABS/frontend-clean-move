...
type QuoteApprovalScheduleStepProps = {
...
export function QuoteApprovalScheduleStep({
  quote,
  onCancel,
  onContinue,
  portalContainer,
}: QuoteApprovalScheduleStepProps) {
  const startsAtId = useId();
  const endsAtId = useId();
  const startsAtDateButtonId = `${startsAtId}-date`;
  const startsAtTimeInputId = `${startsAtId}-time`;
  const endsAtDateButtonId = `${endsAtId}-date`;
  const endsAtTimeInputId = `${endsAtId}-time`;
    ...
    <>
      ...
      <div className="space-y-5 px-5 py-5 sm:px-6">
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-foreground">{quote.customerName}</p>
        ...
        <div className="space-y-2">
          <Label htmlFor={startsAtDateButtonId}>
          ...
          <DatePickerTime
            dateButtonId={startsAtDateButtonId}
            timeInputId={startsAtTimeInputId}
            timeInputAriaLabel="Horário de início do agendamento"
            value={startsAt}
        ...
        <div className="space-y-2">
          <div className="flex min-h-7 items-center justify-between gap-3">
            <Label htmlFor={endsAtDateButtonId}>Término do agendamento</Label>
            {endsAt ? (
              <Button
                type="button"
                variant="outline"
          ...
          <DatePickerTime
            dateButtonId={endsAtDateButtonId}
            timeInputId={endsAtTimeInputId}
            timeInputAriaLabel="Horário de término do agendamento"
            value={endsAt}
            onChange={setEndsAt}
